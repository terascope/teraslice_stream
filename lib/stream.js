'use strict';

const _ = require('lodash');
const { EventEmitter } = require('events');
const { queue, waterfall, constant } = require('async');
const StreamEntity = require('./stream_entity');

const waterfallAsync = pipeline => new Promise((resolve, reject) => {
    waterfall(pipeline, (err, result) => {
        if (err) {
            reject(err);
            return;
        }
        resolve(result);
    });
});

class Stream extends EventEmitter {
    constructor() {
        super();
        this._handleMessage = this._handleMessage.bind(this);
        this._queue = queue(this._handleMessage);
        this._queue.pause();
        this._queue.error = (error) => {
            this._isFailed = true;
            this._queue._tasks.empty();
            this.emit('error', error);
        };
        this._queue.drain = () => {
            if (this._isFull) {
                setTimeout(() => {
                    this._isEnded = true;
                    this.emit('finished');
                }, 50);
            }
            this.emit('drain');
        };
        this.on('ready', () => {
            this._isReady = true;
            this._queue.resume();
        });
        const _ready = () => {
            if (this._isReady) {
                return;
            }
            if (!this._isConsumed) {
                this._isReady = false;
                return;
            }
            this.emit('ready');
        };
        this.on('add:pipeline', (fn) => {
            this._pipeline.push(fn);
            _ready();
        });
        this.on('add:consumer', () => {
            this._isConsumed = true;
            _ready();
        });
        this._isConsumed = false;
        this._isEnded = false;
        this._isFailed = false;
        this._isFull = false;
        this._pipeline = [];
        this._processed = 0;
        this._records = [];
        this._shouldStoreRecords = false;
    }

    done() {
        this.emit('add:consumer');
        return new Promise((resolve, reject) => {
            this.once('error', (err) => {
                reject(err);
            });
            this.once('finished', () => {
                resolve();
            });
        });
    }

    end() {
        this._isFull = true;
        return this;
    }

    filter(fn) {
        this.emit('add:pipeline', async (record) => {
            if (!record) return null;
            if (await fn(record)) {
                return record;
            }
            return null;
        });
        return this;
    }

    map(fn) {
        this.emit('add:pipeline', async (record) => {
            if (!record) return null;
            return fn(record);
        });
        return this;
    }

    each(fn) {
        this.emit('add:pipeline', async (record) => {
            if (!record) return null;
            await fn(record);
            return record;
        });
        return this;
    }

    isPaused() {
        return this._queue.paused;
    }

    pause() {
        this._queue.pause();
        return this;
    }

    resume() {
        this._queue.resume();
        return this;
    }

    async toArray() {
        this._shouldStoreRecords = true;
        await this.done();
        return this._records;
    }

    status() {
        return {
            processed: this._processed,
            finished: _.size(this._records),
            running: this._queue.running(),
            pending: this._queue.length(),
        };
    }

    destroy() {
        this._isReady = false;
        this._isFull = false;
        this._isConsumed = false;
        this._shouldStoreRecords = false;
        this._processed = 0;
        this._records.length = 0;
        this._pipeline.length = 0;
        this._queue.kill();
        return this;
    }

    async write(data, options) {
        if (this._isFailed) {
            return Promise.reject(new Error('Cannot write to stream because it is marked as failed'));
        }
        if (this._isFull) {
            return Promise.reject(new Error('Cannot write to stream because it is marked as full'));
        }
        if (this._isEnded) {
            return Promise.reject(new Error('Cannot write to a stream because it is marked as ended'));
        }
        if (_.isError(data)) {
            this._queue.push(data);
            return null;
        }
        const records = [];
        if (_.isArray(data)) {
            const validRecords = _.every(data, (item) => {
                if (item instanceof StreamEntity) {
                    records.push(item);
                    return true;
                }
                return false;
            });
            if (!validRecords) {
                return Promise.reject(new Error('Stream->write() requires an array of StreamEntities or data'));
            }
        } else if (data instanceof StreamEntity) {
            records.push(data);
        } else {
            records.push(new StreamEntity(data, options));
        }
        if (_.size(records) === 0) {
            return null;
        }
        this._queue.push(records);
        if (_.size(records) === 1) {
            return _.first(records);
        }
        return records;
    }

    async _handleMessage(message) {
        if (_.isError(message)) {
            return Promise.reject(message);
        }
        const pipeline = this._pipeline;
        pipeline.unshift(constant(message));
        const record = await waterfallAsync(pipeline);
        if (this._shouldStoreRecords) {
            if (record) {
                this._records.push(record);
            }
        }
        this._processed += 1;
        return Promise.resolve();
    }
}

module.exports = Stream;
