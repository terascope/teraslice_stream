...
describe('Stream', () => {
    describe('when using a fakeStream', () => {
        let sut;
        const batchSize = 100;
        beforeEach(() => {
    ...
    describe('when using a new stream', () => {
        let sut;
        beforeEach(() => {
            sut = new Stream();
        });
        ...
        it('should process messages asap', () => {
            Promise
                .each(_.times(10), () => sut.write({ now: Date.now() }).delay(100))
                .then(() => sut.end());
            sut.each((r) => {
                const now = Date.now();
                expect(r.data.now).toBeWithinRange(now - 10, now);
            });
            return sut.done();
        });
        describe('->stats', () => {
